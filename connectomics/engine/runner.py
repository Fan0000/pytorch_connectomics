...
class Runner(object):
    r"""Runner class provides pipeline for training and inference.
    ...
    def __init__(self,
        ...
        if self.mode == 'train':
            ...
            if not self.cfg.DATASET.DO_CHUNK_TITLE and not self.inference_singly:
                self.test_filename = self.augmentor.update_name(self.cfg.INFERENCE.OUTPUT_NAME)
    ...
    def get_batch(self, data_iter):
        ...
        volume, target, weight = sample.out_input, sample.out_target_l, sample.out_weight_l
        volume = volume.to(self.device, non_blocking=True)

        data_time = time.perf_counter() - start_time

        return volume, target, weight, data_time

    def train(self):
        r"""Training function of the trainer class.
        """
        if not hasattr(self, "train_loader"):
            self.train_loader = build_dataloader(self.cfg, self.augmentor, self.mode, rank=self.rank)

        train_iter = iter(self.train_loader)

        for i in range(self.start_iter, self.start_iter + self.total_iter_nums):
            start_time = time.perf_counter()
            data_time = self._train_one_batch(train_iter, i)

            if self.is_main_process:
                iter_time = time.perf_counter() - start_time
                self.total_time += iter_time
                avg_iter_time = self.total_time / (i + 1 - self.start_iter)
                est_time_left = avg_iter_time * \
                                (self.total_iter_nums + self.start_iter - i - 1) / 3600.0

                print('[Iteration %05d] Data time: %.4fs, Iter time: %.4fs, Avg iter time: %.4fs, Time Left %.2fh.' % (
                    i, data_time, iter_time, avg_iter_time, est_time_left))
    ...
    def _train_one_batch(self, train_iter, iter_total):
        self.model.train()
        self.optimizer.zero_grad()

        volume, target, weight, data_time = self.get_batch(train_iter)
        ...
        if hasattr(self, 'monitor'):
            ...
            # TO-DO: move visualization to Monitor.update function
            if do_vis:
    ...
    def validate(self, iter_total=0):
        r"""Validation function of the trainer class.
        """
        if self.cfg.DATASET.VAL_IMAGE_NAME is None:
            return

        self.model.eval()

        val_loader = build_dataloader(self.cfg, None, mode='val', rank=self.rank)
        val_iter = iter(val_loader)

        with torch.no_grad():
            val_loss = 0.0
            for i in range(len(val_loader)):
                volume, target, weight, data_time = self.get_batch(val_iter)
    ...
    def _test_one_dataset(self, dataset, test_filename):
        ...
        if self.output_dir is None:
            ...
            writeh5(os.path.join(self.output_dir, test_filename), result,
                    ['vol%d' % (x) for x in range(len(result))])
            print('Prediction saved as: ', test_filename)

    def test(self):
        dir_names = _get_file_list(self.cfg.DATASET.INPUT_PATH)
        input_names = _get_file_list(self.cfg.DATASET.IMAGE_NAME)
        assert len(dir_names) == 1

        n_inputs = len(input_names)
        n_digits = int(math.log10(n_inputs)) + 1

        for i, name in enumerate(input_names):
            dataset = get_dataset(
                self.cfg, self.augmentor, self.mode, self.rank,
                dir_name_init=dir_names, img_name_init=name)

            test_filename = self.augmentor.update_name(
                f"{self.cfg.INFERENCE.OUTPUT_NAME}_{str(i).zfill(n_digits)}.h5"
            )

            self._test_one_dataset(dataset, test_filename)
    ...
    # TO-DO: rename the function name?
    # update_swa_params?
